...
function tableSortValue(text) {
  ...
  if (dateMatch) {
    const dateText = dateMatch[1].length === 4 ? `${dateMatch[1]}-${dateMatch[2]}-${dateMatch[3]}` : `${dateMatch[3]}-${dateMatch[2]}-${dateMatch[1]}`;
...
function sortTableByColumn(table, columnIndex, forcedDir) {
  ...
  rows
    .sort((a, b) => {
      ...
      const comparison = aValue.type === "text" || bValue.type === "text"
